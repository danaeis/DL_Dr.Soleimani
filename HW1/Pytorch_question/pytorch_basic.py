...
def mutate_tensor(x, indices, values):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  values_tensor = torch.tensor(values, device=x.device, dtype=x.dtype)
  indices_tensor = torch.tensor(indices, device=x.device, dtype=torch.long).t()
  # Mutate tensor using index_put_
  x.index_put_(tuple(indices_tensor), values_tensor, accumulate=False)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def make_one_hot(x):
  ...
  y = mutate_tensor(y,
                    [[i, val] for i, val in enumerate(x)],
                    [1]*N)
...
def reshape_practice(x):
  ...
  # Replace "pass" statement with your code
  y = x.view(6,4)
  row_order = [0, 3, 1, 4, 2, 5]  # Specify the desired order of rows

  # Reorder the rows of the tensor
  reordered_tensor = y[row_order]
  y = reordered_tensor.view(3,8)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  ind_min = torch.min(x,1).indices
  ind_min = ind_min.reshape(len(ind_min),1)
  indexs = torch.tensor(range(ind_min.shape[0])).reshape(ind_min.shape)
  indices = torch.cat((indexs,ind_min),axis=1)
  values = torch.zeros(ind_min.shape[0])
  y = mutate_tensor(x,indices,values)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def normalize_columns(x):
  ...
  sigma = torch.sqrt(torch.sum(diff_squared, dim=0) / x.shape[0])
  print("sigma", sigma)
  # Normalize the columns of x
  y = (x - mu) / sigma
  y = torch.round(y)
  # Replace NaN elements with the corresponding column mean (mu)
  nan_mask = torch.isnan(y)
  for col_idx in range(x.shape[1]):
      y[nan_mask[:, col_idx], col_idx] = mu[col_idx]
